...
switch ($action) {
    ...
    case "update_course":
        // Handle the form submission for updating a course
        if ($course_id && !empty($course_name)) {
            // Call the function to update the course in the database
            update_course($course_id, $course_name);
            header("Location: .?action=list_courses");
            exit();
        } else {
            $error = "Invalid course data. Please check the fields and try again.";
            include("view/error.php");
            exit();
        }
        break;
